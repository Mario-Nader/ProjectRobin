...
async function getharvest(req,res){
  ...
  res.status(200).send({"patrols":pats})
}catch(err){
  console.log(err)
  res.status(500).send({message:"error in getting harverst data please try again later"})
}
}

async function watering(req,res){
  let id = req.id
  let chef = await scouts.findById(id).exec()
  if(!chef){
  ...
  if(patrol){
  if(patrol.farming){
    res.status(400).send({message:"this patrol has already watered their plants"})
  }else{
    patrol.farming = true
    await patrol.save()
  ...
  res.status(403).send({message:"must be a chef to water the plants"})
}
}else{
  res.status(400).send({message:"this patrol doesn't exist"})
}
}
...
async function harvest() {
  ...
  let landArr  = await lands.find({patrol_ID : {$ne : kadrID}})//reduce the number of lands by excluding the leaders' lands
...
module.exports = {view_scores,update_scores,getharvest,harvest}